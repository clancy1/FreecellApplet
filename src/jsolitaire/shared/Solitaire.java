...
/** Do general initialization and call game-specific initialization function*/
    public void init()
    {
    	...
    	try
    	{
        	...
        	{
        		machineId += String.format("%02X%s", mac[k], (k < mac.length - 1) ? "-" : "");
    	...
    	buttons     = btns;
    	buttonPanel = makeButtonPanel(buttons, new GridLayout(5, 1, 0, 0), bpWidth, Card.HEIGHT);
 ...
 *  @return coordinates of lower right of component */ 
    ...
    {
        add(c); 
        c.move(x + insets.left, y + insets.top);
        ...
        {   this.resize(p.x + 5, appletSize.height);
            appletSize = this.size();
        }
        if(p.y + 5 > appletSize.height)
            this.resize(appletSize.width, p.y + 5); 
        return p; 
    }
 ...
 *   the state of the corresponding checkbox */
    ...
    /**
     * Event to handle mouse movement so that it can be placed into the log file.
     */
 ...
 *   @param btn button which made move, null if cards moved normally */
                         ...
                         int insI, int delI, Button btn)
    {
    	...
    	for (int i = 0; i < stacks.length; i++)
    	{
    		if (src.equals(stacks[i]))
    		{
    			srcStack = i;
    		}
    		else if (dest.equals(stacks[i]))
    		{
    			destStack = i;
    		}
    	}
 ...
 *   @return the enclosing frame */
    ...
    // Protected method for logging messages to the server.
    // Sends the message to the log file created by the game, silently capturing any errors that come back.
    protected void logMessage(String msg) 
    {
    	logFile = machineId + "_" + playerId + "_" + sessionId + "_" + gameType + "_" + gameId + ".txt";
    	String eol = System.getProperty("line.separator");
